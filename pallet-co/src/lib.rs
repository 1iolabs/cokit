...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn push_reference(origin: OriginFor<T>, key: Vec<u8>, reference: Vec<u8>) -> DispatchResult {
			...
			// get list reference
			let reference_as_cid = Cid::try_from(reference).map_err(|_e| Error::<T>::InvalidReference)?;
			let list_reference = match <References<T>>::try_get(&key) {
				Ok(next) => ListReference {
					version: Default::default(),
					reference: reference_as_cid,
					next: Some(Cid::try_from(next).map_err(|_e| Error::<T>::InvalidKey)?),
				},
				Err(_) => ListReference { version: Default::default(), reference: reference_as_cid, next: None },
			};

			// encode list reference
			let (cid, data): (Cid, Vec<u8>) = list_reference.try_into().map_err(|_e| Error::<T>::GenericEncoding)?;
			let value = cid.to_bytes();

			// apply offchain index (CID => DATA)
			// this will safed to IPFS by offchain worker later.
