...
use sc_service::{
	config::{
		ExecutionStrategy, KeystoreConfig, NetworkConfiguration, NodeKeyConfig, OffchainWorkerConfig,
...
pub fn co_execute<O: Observer<CoAction, Infallible> + 'static>(
	...
	actions
		...
		.flat_map(move |(id, state)| {
			...
			let running = CoAction::CoExecuteStateChanged { id: id.clone(), state: CoExecuteState::Running };
			let stopped = CoAction::CoExecuteStateChanged { id: id.clone(), state: CoExecuteState::Stopped };
...
impl CoRunner {
	...
	fn network_config(&self, settings: &CoSettings, node_key: NodeKeyConfig) -> Result<NetworkConfiguration> {
		...
		network_config.listen_addresses = vec![iter::once(multiaddr::Protocol::Ip4(Ipv4Addr::new(127, 0, 0, 1)))
	...
	fn configuration(&self, settings: &CoSettings) -> Result<Configuration> {
		...
		Ok(Configuration {
			...
			trie_cache_maximum_size: get_int_setting::<usize>(settings, "substrate.trie_cache_maximum_size"),
			state_pruning: None,
			...
			max_runtime_instances: get_int_setting(settings, "substrate.max_runtime_instances").unwrap_or(8),
			announce_block: true,
