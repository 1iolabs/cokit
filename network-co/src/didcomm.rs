...
use libp2p::{PeerId, swarm::{NetworkBehaviour, derive_prelude::{ConnectionId, ConnectionEstablished}, ToSwarm, NotifyHandler, dial_opts::DialOpts, PollParameters, THandlerInEvent, ConnectionDenied, THandler, FromSwarm, THandlerOutEvent, ConnectionClosed, DialFailure, AddressChange}, Multiaddr, core::{Endpoint, ConnectedPoint}};
        ...
        match event {
            handler::Event::Received { message } => {
                self.pending_events.push_back(ToSwarm::GenerateEvent(
                    Event::Received { peer_id: peer, message })
                );
            },
            handler::Event::Sent { message } => {
                self.pending_events.push_back(ToSwarm::GenerateEvent(
                    Event::Sent { peer_id: peer, message })
                );
            },
