...
pub fn heads_message_heads(
	...
	actions
		.filter_map(move |action| {
			ready(match action {
				Action::HeadsMessageReceived { from: _, peer, message_id, message: HeadsMessage::Heads(co, heads) } => {
					Some((message_id, peer, co, heads))
				},
				_ => None,
			})
		})
		.then(move |(message_id, peer, co, heads)| {
			let context = context.clone();
			async move { join_heads(context, message_id, peer, co, heads).await }
		})
...
pub fn heads_message_heads_request(
	...
	actions
		.filter_map(move |action| {
			ready(match action {
				Action::HeadsMessageReceived { from, peer, message_id, message: HeadsMessage::HeadsRequest(co) } => {
					Some((message_id, from, peer, co))
				},
				_ => None,
			})
		})
		.then(move |(message_id, from, peer, co)| {
			let context = context.clone();
			async move { request_heads(context, message_id, from, peer, co).await }
		})
