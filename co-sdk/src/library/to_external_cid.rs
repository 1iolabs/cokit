...
pub async fn to_external_cids_opt(
	mapping: &impl BlockStorageContentMapping,
	cids: BTreeSet<Cid>,
) -> Option<BTreeSet<Cid>> {
	if mapping.is_content_mapped().await {
		...
		for cid in cids {
			result.insert(to_external_cid_opt(mapping, Some(cid)).await?);
		}
		Some(result)
	} else {
		Some(cids)
...
/// If some [`Cid`] could not be mapped return [`None`].
/// If mapping is not enabled return the original Cids.
pub async fn to_external_cids_opt_map(
	mapping: &impl BlockStorageContentMapping,
	cids: BTreeSet<Cid>,
	...
	if mapping.is_content_mapped().await {
		...
		for cid in cids {
			result.insert(cid, to_external_cid_opt(mapping, Some(cid)).await?);
		}
		Some(result)
	} else {
