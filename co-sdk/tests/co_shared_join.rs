...
async fn test_co_shared_join() {
	...
	shared_co
		...
		.unwrap();
	shared_co
		.push(&identity1, CO_CORE_NAME_CO, &CoAction::ParticipantJoin { participant: identity2.identity().to_owned() })
		.await
		.unwrap();
	println!("8");
	...
	let (_, key) = state::find(&shared_co.storage(), &key_store.keys, |(k, _)| k == key_reference)
		.await
		.unwrap()
		.unwrap();
	...
	SharedCoJoin::new(shared_co.id().clone())
		.with_trusted_peer(peer2.application.network().map(|network| network.peer_id()).unwrap())
