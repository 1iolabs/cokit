...
} from "../../group-view-add-participant-dialog-plugin/actions/index.js";
import { addParticipantDialogPluginId } from "../../group-view-add-participant-dialog-plugin/types/plugin.js";
import {
  LeaveGroupDialogActionType,
  LeaveGroupDialogLeaveAction,
} from "../../group-view-leave-group-dialog-plugin/actions/index.js";
import {
  LeaveGroupDialogGroupNameTag,
  leaveGroupDialogPluginId,
} from "../../group-view-leave-group-dialog-plugin/types/tag.js";
import {
  RemoveParticipantDialogActionType,
  RemoveParticipantDialogRemoveAction,
} from "../../group-view-remove-participant-dialog-plugin/actions/index.js";
import {
  removeParticipantDialogPluginId,
  RemoveParticipantDialogRequiredTags,
} from "../../group-view-remove-participant-dialog-plugin/types/plugin.js";
import {
...
export const openInviteParticipantDialogEpic: GroupViewEpicType = (action$, _, context) =>
  action$.pipe(
...
export const participantAddedEpic: GroupViewEpicType = (action$, state$, context) =>
  action$.pipe(
...
export const openRemoveParticipantDialogEpic: GroupViewEpicType = (action$, state$, context) =>
  action$.pipe(
    filter(
      (action): action is GroupViewRemoveParticipantAction =>
        action.type === GroupViewPluginActionType.RemoveParticipant,
    ),
    ...
    mergeMap(async ([action, state]) => {
      ...
      if (action.payload.isYou) {
        ...
        const [loadAction, pluginId] = await api.loadDialog(
          removeParticipantDialogPluginId,
          removeParticipantDialogTags,
        );
...
export const leftGroupEpic: GroupViewEpicType = (action$, state$, context) =>
  action$.pipe(
    filter(isPluginAction),
    map((action) => {
      return action.payload;
...
export const participantRemovedEpic: GroupViewEpicType = (action$, state$, context) =>
  action$.pipe(
    filter(isPluginAction),
    map((action) => {
      return action.payload;
    ...
    filter(
      (action): action is RemoveParticipantDialogRemoveAction =>
        action.type === RemoveParticipantDialogActionType.Remove,
    ),
    withLatestFrom(state$),
    mergeMap(async ([action, state]) => {
      if (!state.isNew) {
      ...
      return [
        identity<GroupViewParticipantRemovedAction>({
          payload: { participant: action.payload.did },
