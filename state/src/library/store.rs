...
where
	R: Reducer + Send + 'static,
{
	pub fn new(state: R::State, on_changed: impl Fn(&R::State) + Send + 'static, reducer: R) -> Self {
		Self {
	...
	R: Reducer + Send + 'static,
{
	fn dispatch(&mut self, action: R::Action) {
		self.state = Some(
			self.reducer
				.reduce(self.state.take().unwrap_or_else(|| self.state_default.clone()), &action),
		);
	...
	fn state(&self) -> R::State {
		...
		self.state.clone().expect("Calling state() while dispatching is not allowed")
	}
	...
	fn with_middleware(
		...
		Box::new(MiddlewareStore { middleware, next: self })
	}
}
...
{
	fn dispatch(&mut self, action: R::Action) {
		self.middleware.as_mut().dispatch(self.next.as_mut(), action);
	}
	...
	fn middleware() {
			...
			R::State: Clone,
		{
			fn dispatch<'a>(&mut self, next: &'a mut dyn StoreApi<R>, action: R::Action) {
				...
				self.states.send((action, previous_state, next_state)).unwrap();
			}
		}
