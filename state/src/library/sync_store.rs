    ...
    R::State: SyncState,
{
    pub fn new(state: R::State, reducer: R) -> Self {
        ...
        let on_changed = move |state: &R::State| {
            tx.send(state.clone())
                .expect("Send state subscription failed")
        };
        Self {
            store: Arc::new(Mutex::new(Box::new(Store::new(
                state.clone(),
                on_changed,
                reducer,
            )))),
...
#[cfg(test)]
mod tests {
    use crate::SyncStoreApi;
    use crate::{FnReducer, Reducer, SyncStore};
    use rxrust::prelude::*;
    ...
    impl Reducer for TestState {
        ...
        fn reduce(&self, state: TestState, action: &TestAction) -> TestState {
            match action {
                TestAction::IncCount => TestState {
                    count: state.count + 1,
                    ..state
                },
                TestAction::DecCount => TestState {
                    count: state.count - 1,
                    ..state
                },
            }
