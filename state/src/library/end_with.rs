	...
	O: Observer<Item, Err>,
{
	...
	fn actual_subscribe(self, observer: O) -> Self::Unsub {
		self.source
			.actual_subscribe(EndWithObserver { next: MutRc::own(Some(observer)), values: self.values })
	}
}
	...
	O: Observer<Item, Err>,
{
	...
	fn is_finished(&self) -> bool {
		self.next.rc_deref().as_ref().map_or(true, |data| data.is_finished())
	}
}
...
impl<Item, Err, S> EndWithExt<Item, Err> for S where S: ObservableExt<Item, Err> {}

#[cfg(test)]
mod test {
	use super::EndWithExt;
	use rxrust::{of_sequence, prelude::*};
