...
where
    ...
    E::Result: Observable<
            R::Action,
            Infallible,
            BoxObserver<'static, R::Action, Infallible>,
            Unsub = E::Unsub,
        > + BoxIt<BoxOp<'static, R::Action, Infallible>>,
    // <<E as Epic<R, C, BoxObserverThreads<<R as Reducer>::Action, Infallible>>>::Result as Observable::<R::Action, Infallible, BoxObserverThreads<R::Action, Infallible>>>::Unsub: Send + 'static,
{
    fn box_epic(
        &self,
        actions: ActionObservable<R::Action>,
    ...
    R: Reducer + 'static,
{
    fn dispatch<'a>(&mut self, next: &'a mut dyn StoreApi<R>, action: R::Action) {
        ...
        // epic
        self.commands_tx
            .send(EpicCommand::State(next.state(), action))
            .unwrap();
    }
}
