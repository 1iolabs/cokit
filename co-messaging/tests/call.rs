use eco_message_event_format::{
    call_event::{
        AnswerCallContent, CallCandidatesContent, CallInviteContent, CallNegotiationContent,
        HangupCallContent, HangupCallReason, ICECandidate, RejectCallContent,
        SelectCallAnswerContent,
    },
...
#[test]
fn init_call() {
    let mut event_content = CallInviteContent::new(
        "call_1",
        "some_device_address",
        None,
        10000,
...
fn answer_call() {
    let event_content =
        AnswerCallContent::new("call_1", "some_other_device_address", "some_sdp_string");
    let event = MatrixEvent::new(
        "some_event",
        1577836800000,
        "some_room",
...
fn call_candidates() {
    let event_content = CallCandidatesContent::new(
        ...
        "some_device_address",
        vec![
            ICECandidate::new("some_candidate", "video", 0),
            ICECandidate::new("some_other_candidate", "audio", 1),
        ],
    );
    let event = MatrixEvent::new(
        "some_event",
...
fn select_call_answer() {
    let event_content =
        SelectCallAnswerContent::new("call_1", "some_device_address", "some_other_device_address");
    let event = MatrixEvent::new(
        "some_event",
        1577836800000,
        "some_room",
...
fn call_negotiation() {
    ...
    let mut event_content_offer = CallNegotiationContent::offer(
        "call_1",
        "some_device_address",
        "some_sdp_offer_string",
        10000,
    );
    ...
    let mut event_content =
        CallNegotiationContent::answer("call_1", "some_device_address", "some_sdp_answer_string");
    // assert that offer fields are not set
...
#[test]
fn hangup_call() {
    let event_content = HangupCallContent::new(
        "call_1",
        "some_device_address",
        HangupCallReason::UserHangup,
    );
    let event = MatrixEvent::new(
        "some_event",
        1577836800000,
