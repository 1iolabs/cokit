...
fn test_text_content() {
    ...
    event_content.mentions = Mentions {
        user_ids: vec!["did:some:user".into()],
    }
    .into();
...
fn test_poll_start() {
    let mut event_content =
        poll_event::PollStartContent::new("What?", vec![], poll_event::PollKind::Anonymous);
...
fn test_poll_response() {
    let mut event_content =
        poll_event::PollResponseContent::new("response", vec![], "some_poll_start");
    event_content.add_answer(PollAnswer::new("test", "Test"));
    let event = MatrixEvent::new(
        "some_poll_response",
        1577836800000,
    ...
    assert_eq!(event.event_type(), "m.room.message");
    assert_eq!(
        event.content.generate_relation_type(),
        Some("m.poll".into())
    );
    let json = serde_json::to_string_pretty(&event).unwrap();
    println!("JSON: {}", json);
...
fn test_annotation() {
    let event_content =
        ReactionContent::new(RelatesTo::annotation("some_other_event", "thumbs_up"));
    let event = MatrixEvent::new(
        "some_event",
        1577836800000,
        "@some.room",
