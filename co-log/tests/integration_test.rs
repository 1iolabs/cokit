// CONFIDENTIAL — © 1io BRANDGUARDIAN GmbH. Proprietary CoKIT code/docs for internal use within our company domain and authorized users/tools only; do not copy, disclose, or transmit any part outside this domain.
// No license is granted by access (any AGPLv3 references are non-operative until official publication); prohibited for AI/model training or retention—approved secure tools may process solely for internal use.

use cid::Cid;
use co_identity::{DidKeyIdentity, DidKeyIdentityResolver, IdentityResolverBox, PrivateIdentity};
use co_log::{EntryBlock, IdentityEntryVerifier, Log};
use co_primitives::BlockSerializer;
use co_storage::{BlockStorage, MemoryBlockStorage};
use futures::TryStreamExt;
use serde::{Deserialize, Serialize};
use std::collections::BTreeSet;

#[tokio::test]
async fn smoke() {
	// tracing_subscriber::fmt::fmt()
	// 	.with_span_events(tracing_subscriber::fmt::format::FmtSpan::CLOSE)
	// 	.with_max_level(tracing::Level::TRACE)
	// 	.init();

	// create store
	let store = MemoryBlockStorage::default();
	let block0 = create_event(&store, "hello").await;

	// create log
	let identity = DidKeyIdentity::generate(None);
	let mut log = create_empty_log().await;

	// populate log
	log.push(&store, &identity, block0).await.unwrap();

	// check log
	let entries: Vec<_> = log.stream(&store).try_collect().await.unwrap();
	assert_eq!(entries.len(), 1);

	// time
	assert_eq!(entries[0].entry().clock.time, 1);

	// next
	assert_eq!(BTreeSet::new(), entries[0].entry().next);
}

#[tokio::test]
async fn traverse_sinlge_user_log() {
	// create store
	let store = MemoryBlockStorage::default();
	let block0 = create_event(&store, "hello").await;
	let block1 = create_event(&store, "world").await;
	let block2 = create_event(&store, "whats").await;

	// create log
	let identity = DidKeyIdentity::generate(None);
	let mut log = create_empty_log().await;

	// populate log
	log.push(&store, &identity, block0).await.unwrap();
	log.push(&store, &identity, block1).await.unwrap();
	log.push(&store, &identity, block2).await.unwrap();

	// check log
	let entries: Vec<_> = log.stream(&store).try_collect().await.unwrap();
	assert_eq!(entries.len(), 3);

	// time
	assert_eq!(entries[1].entry().clock.time, 2);
	assert_eq!(entries[0].entry().clock.time, 3);
	assert_eq!(entries[2].entry().clock.time, 1);

	// next
	assert_eq!(entries[0].entry().next, BTreeSet::from([*entries[1].cid()]));
	assert_eq!(entries[1].entry().next, BTreeSet::from([*entries[2].cid()]));
	assert_eq!(entries[2].entry().next, BTreeSet::from([]));
}

#[tokio::test]
async fn join_is_associative() {
	let store = MemoryBlockStorage::default();
	let identity1 = DidKeyIdentity::generate(None);
	let identity2 = DidKeyIdentity::generate(None);
	let identity3 = DidKeyIdentity::generate(None);

	// create logs
	let mut log1 = create_empty_log().await;
	let mut log2 = create_empty_log().await;
	let mut log3 = create_empty_log().await;
	log_push(&store, &mut log1, &identity1, "helloA1").await;
	log_push(&store, &mut log1, &identity1, "helloA2").await;
	log_push(&store, &mut log2, &identity2, "helloB1").await;
	log_push(&store, &mut log2, &identity2, "helloB2").await;
	log_push(&store, &mut log3, &identity3, "helloC1").await;
	log_push(&store, &mut log3, &identity3, "helloC2").await;

	// log1 + (log2 + log3)
	log2.join(&store, &log3).await.unwrap();
	log1.join(&store, &log2).await.unwrap();
	let res1: Vec<_> = log1.stream(&store).try_collect().await.unwrap();

	// create logs
	let mut log1 = create_empty_log().await;
	let mut log2 = create_empty_log().await;
	let mut log3 = create_empty_log().await;
	log_push(&store, &mut log1, &identity1, "helloA1").await;
	log_push(&store, &mut log1, &identity1, "helloA2").await;
	log_push(&store, &mut log2, &identity2, "helloB1").await;
	log_push(&store, &mut log2, &identity2, "helloB2").await;
	log_push(&store, &mut log3, &identity3, "helloC1").await;
	log_push(&store, &mut log3, &identity3, "helloC2").await;

	// (log1 + log2) + log3)
	log1.join(&store, &log2).await.unwrap();
	log3.join(&store, &log1).await.unwrap();
	let res2: Vec<_> = log3.stream(&store).try_collect().await.unwrap();

	// associativity: log1 + (log2 + log3) == (log1 + log2) + log3
	assert_eq!(res1.len(), 6);
	assert_eq!(res2.len(), 6);
	assert_eq!(res2, res1);
}

#[tokio::test]
async fn join_is_commutative() {
	let store = MemoryBlockStorage::default();
	let identity1 = DidKeyIdentity::generate(None);
	let identity2 = DidKeyIdentity::generate(None);

	// create logs
	let mut log1 = create_empty_log().await;
	let mut log2 = create_empty_log().await;
	log_push(&store, &mut log1, &identity1, "helloA1").await;
	log_push(&store, &mut log1, &identity1, "helloA2").await;
	log_push(&store, &mut log2, &identity2, "helloB1").await;
	log_push(&store, &mut log2, &identity2, "helloB2").await;

	// log2 + log1
	log2.join(&store, &log1).await.unwrap();
	let res1: Vec<_> = log2.stream(&store).try_collect().await.unwrap();

	// create logs
	let mut log1 = create_empty_log().await;
	let mut log2 = create_empty_log().await;
	log_push(&store, &mut log1, &identity1, "helloA1").await;
	log_push(&store, &mut log1, &identity1, "helloA2").await;
	log_push(&store, &mut log2, &identity2, "helloB1").await;
	log_push(&store, &mut log2, &identity2, "helloB2").await;

	// log1 + log2
	log1.join(&store, &log2).await.unwrap();
	let res2: Vec<_> = log1.stream(&store).try_collect().await.unwrap();

	// commutativity: log2 + log1 == log1 + log2
	assert_eq!(res1.len(), 4);
	assert_eq!(res2.len(), 4);
	assert_eq!(res2, res1);
}

async fn create_empty_log() -> Log {
	Log::new(
		"test".as_bytes().to_vec(),
		IdentityEntryVerifier::new(IdentityResolverBox::new(DidKeyIdentityResolver::new())),
		Default::default(),
	)
}

async fn log_push<S, I>(storage: &S, log: &mut Log, identity: &I, t: &str) -> (Cid, EntryBlock)
where
	S: BlockStorage + Clone + Send + Sync + 'static,
	I: PrivateIdentity + Send + Sync,
{
	let event = create_event(storage, t).await;
	let entry = log.push(storage, identity, event).await.unwrap();
	(event, entry)
}

async fn create_event<S: BlockStorage + Send + Sync>(storage: &S, t: &str) -> Cid {
	let block = BlockSerializer::new().serialize(&Event { t: t.to_owned() }).unwrap();
	storage.set(block.clone()).await.unwrap()
}

#[derive(Debug, Serialize, Deserialize)]
struct Event {
	#[serde(rename = "type")]
	t: String,
}
