...
This allows for the explicit configuration of DIDs with which to connect, instead of (by default) all participants in a CO.
...
### Peer
Directly-configured endpoints (IP/DNS).
...
The libp2p ping protocol is different from the ping command line utility, as it requires an already established libp2p connection.
...
The didcomm messages denote a connection request from one CO participant to another.  
...
Whenever a CO (specifically its heads) changes, a new gossip message with the changed heads will be issued.
